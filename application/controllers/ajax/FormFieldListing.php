...
class formFieldListing extends CI_Controller
{
    ...
    function getFormField()
    {
        ...
        $updatePermission = getDataByVal('update_data', 'permission', array('employee_id' => $id, 'type' => 'form_fields'));

        foreach ($_POST as $key => $value) {
            $data[$key] = $value;
        }
        $query = $this->loginModel->searchData('form_fields', $data);
        ...
        foreach ($query->result() as $r) {
            ...
            $data[] = array(
                ...
                $status,
                $updateButton,
                getDataByVal('name', 'shop_type', array('id' => $r->super_category_id)),
                $r->field_name,
                $r->field_type,
                $r->field_validation,
            );
