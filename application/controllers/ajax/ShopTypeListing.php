...
class ShopTypeListing extends CI_Controller
{
    ...
    function getShopType()
    {
        ...
        $updatePermission = getDataByVal('update_data', 'permission', array('employee_id' => $id, 'type' => 'shop_type'));

        foreach ($_POST as $key => $value) {
            $data[$key] = $value;
        }
        $query = $this->loginModel->searchData('shop_type', $data);
        ...
        foreach ($query->result() as $r) {
            ...
            $image = '<img src="' . base_url() . 'image/shop_type/' . $r->image . '" class="img img-circle img-thumbnail img-open" style="width:50px;">';
            $data[] = array(
                $sno,
                $checbox,
                $status,
