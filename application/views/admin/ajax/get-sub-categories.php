    ...
    foreach ($categoryData->result() as $categoryObj) {
        ...
        <div class="col-sm-2 text-center p-2">
            <div class="p-3" style="box-shadow: 0px 0px 3px 2px #cfccc3;border-radius: 5px;"
                onclick="createProduct('<?php echo id_encode($categoryObj->id) ?>');">
                <img src="<?php echo base_url() . 'image/sub_category/' . $categoryObj->image ?>"
                ...
                <button type="button" class="btn btn-info btn-xs"
                    onclick="createProduct('<?php echo id_encode($categoryObj->id) ?>');">Select</button>
            </div>
        </div>
        <?php
    }
